...
**Q: Does CCR work with the free Claude plan?**
A: CCR requires Claude Code, which requires Claude Max ($20/mo) or an API key. The Max
subscription is the recommended path — no per-token billing.

**Q: Will Claude automatically commit my progress?**
A: Yes, if you ran `ccr install`. The `PostToolUse` and `Stop` hooks accumulate session
activity and auto-commit when you end the session. You can also call `gcc_commit` manually
for finer-grained control (recommended for experiments).
